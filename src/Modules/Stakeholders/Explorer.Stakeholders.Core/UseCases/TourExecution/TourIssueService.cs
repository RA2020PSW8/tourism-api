    ...
    public class TourIssueService : CrudService<TourIssueDto, TourIssue>, ITourIssueService
    {
        ...
        public Result<TourIssueDto> SetResolvedDateTime(TourIssueDto tourIssue)
        {
            try
            {
                ...
                String additionalMessage = String.Format("{0:dd/MM/yyyy}", tourIssue.ResolveDateTime) +  " for the tour named '" + tour.Name + "'.";

                _notificationService.Generate(tour.UserId, NotificationType.ISSUE_DEADLINE, url, DateTime.UtcNow, additionalMessage);
