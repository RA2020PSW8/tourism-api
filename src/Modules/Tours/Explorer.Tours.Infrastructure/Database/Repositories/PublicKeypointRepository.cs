...
namespace Explorer.Tours.Infrastructure.Database.Repositories
{
    public class PublicKeypointRepository : CrudDatabaseRepository<PublicKeypoint, ToursContext>, IPublicKeypointRepository
    {
        ...
        public PagedResult<PublicKeypoint> GetPagedInRange(int page, int pageSize, double longitude, double latitude, double radius)
        {
            ...
            var task = _dbSet
                .Where(kp =>
                    ...
                    Math.Atan2(
                        Math.Sqrt(
                            Math.Sin((kp.Latitude * degreeRad - latDegree) / 2) * Math.Sin((kp.Latitude * degreeRad - latDegree) / 2)
                            +
                            Math.Cos(kp.Latitude * degreeRad) * Math.Cos(latDegree)
                            *
                            Math.Sin((kp.Longitude * degreeRad - longDegree) / 2) * Math.Sin((kp.Longitude * degreeRad - longDegree) / 2)
                            )
                        ,
                        Math.Sqrt(
                            1
